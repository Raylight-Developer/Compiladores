...
class Symbol_Table(QTableWidget):
	def __init__(self, type: str):
		...
		if type == "Classes":
			self.columns = ["ID", "TAC", "TAC", "Init?", "Fun", "Var", "Scope", "S", "D"]
		if type == "Functions":
			self.columns = ["ID", "TAC", "TAC", "Return", "Origin", "Params", "Code", "Scope", "S", "D"]
		if type == "Variables":
			self.columns = ["ID", "TAC", "TAC", "Type", "Origin", "Code", "Scope", "S", "D"]
	...
	def addSymbol(self, value: Class | Function | Variable):
		...
		if isinstance(value, Class):
			...
			self.setItem(row, 6, QTableWidgetItem(value.parent.ID if value.parent else 'Global'))
			self.setItem(row, 7, QTableWidgetItem(str(value.scope_depth_count)))
			...
			self.setItem(row, 7, QTableWidgetItem(value.member.ID if value.member else "Global"))
			self.setItem(row, 8, QTableWidgetItem(str(value.scope_depth_count if not value.member else "-")))
			...
			self.setItem(row, 6, QTableWidgetItem(value.member.ID if value.member else "Global"))
			self.setItem(row, 7, QTableWidgetItem(str(value.scope_depth_count) if not value.member else "-"))
			self.setItem(row, 8, QTableWidgetItem(str(value.scope_depth) if not value.member else "-"))
